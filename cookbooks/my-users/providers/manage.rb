...
def user_list(data_bag_name)
  user_items = []
  data_bag(data_bag_name).each do |user|
    user_items << Chef::EncryptedDataBagItem.load(data_bag_name, user)
  end
  return user_items
end
...
action :remove do
  user_list(new_resource.data_bag).each do |rm_user|
    user rm_user['id'] do
      action :remove
      only_if {rm_user['groups'].include? new_resource.search_group and rm_user['action'] == "remove"}
    end
  end
end
...
action :create do
  ...
  user_list(new_resource.data_bag).each do |u|
    if u['groups'].include? new_resource.search_group and not u['action'] == "remove"
      ...
      template "#{home_dir}/.ssh/authorized_keys" do
        ...
        only_if {u['ssh_keys'] && node['roles'].include?("startup")}
      end
      if u['ssh_private_key']
        ...
        template "#{home_dir}/.ssh/id_#{key_type}" do
          ...
          only_if {node['roles'].include?("startup")}
        end
      end
      if u['ssh_public_key']
